...
void fixed_color(DeviceInfo& dev) {
  dev.strip.fill(Adafruit_NeoPixel::Color(dev.fixedColorData.color.r, dev.fixedColorData.color.g, dev.fixedColorData.color.b), 0, dev.defaultData.ledLenght);
...
void rainbow(DeviceInfo& dev) {
  ...
  firstPixelHue = int(((int(millis() * 3 * dev.rainbowData.velocity / 100) % 1000) / 1000.0) * 65535L);
  ...
  for (int i = 0; i < dev.strip.numPixels(); i++)
  {
    ...
    dev.strip.setPixelColor(i, dev.strip.gamma32(dev.strip.ColorHSV(pixelHue, 255, dev.defaultData.brightness)));
  }
}
